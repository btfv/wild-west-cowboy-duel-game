...
use crate::client::constants::{GAME_W, GAME_H};
...
pub fn text_params<'a>(size: f32, color: Color, font: &'a Font) -> TextParams<'a> {
    TextParams { font: Some(font), font_size: FONT_ATLAS_SIZE, font_scale: font_scale(size), color, ..Default::default() }
}
...
pub fn draw_centered(text: &str, y: f32, size: f32, color: Color, font: &Font) {
    ...
    draw_text_ex(text, GAME_W / 2.0 - dim.width / 2.0, y, text_params(size, color, font));
}

pub fn hovered_text(text: &str, x: f32, y: f32, size: f32, ox: f32, oy: f32, scale: f32, font: &Font) -> bool {
    let dim = measure_text(text, Some(font), FONT_ATLAS_SIZE, font_scale(size));
...
pub fn draw_link(text: &str, x: f32, y: f32, size: f32, ox: f32, oy: f32, scale: f32, font: &Font) -> bool {
    let dim = measure_text(text, Some(font), FONT_ATLAS_SIZE, font_scale(size));
    let lx = x - dim.width / 2.0;
    let hovered = hovered_text(text, lx, y, size, ox, oy, scale, font);
    txt(text, lx, y, size, if hovered { YELLOW } else { Color::from_rgba(100, 180, 255, 255) }, font);
    hovered && is_mouse_button_pressed(MouseButton::Left)
}

pub fn set_game_camera(ox: f32, oy: f32, sw: f32, sh: f32, scale: f32) {
    let mut cam = Camera2D::from_display_rect(Rect::new(-ox / scale, -oy / scale, sw / scale, sh / scale));
...
fn lcg(s: u32) -> u32 { s.wrapping_mul(2654435761).wrapping_add(1013904223) }
...
pub fn draw_background(seed: u32) {
    ...
    for w in sky_bands.windows(2) {
        ...
        for row in py0..py1 {
            ...
            draw_line(0.0, row as f32, GAME_W, row as f32, 1.0, Color::from_rgba(r, g, b, 255));
        }
    }

    for col in 0..16i32 {
        let alpha = ((16 - col) as f32 / 16.0 * 80.0) as u8;
        draw_line(col as f32, 0.0, col as f32, GAME_H, 1.0, Color::from_rgba(0, 0, 0, alpha));
        draw_line(GAME_W - col as f32, 0.0, GAME_W - col as f32, GAME_H, 1.0, Color::from_rgba(0, 0, 0, alpha));
    }
    ...
    for row in 0..rows {
        for col in 0..cols {
            ...
            if r > 0 || b > 0 {
                draw_rectangle(col as f32 * 8.0, row as f32 * 8.0, 8.0, 8.0, Color::from_rgba(r, 0, b, 30));
            }
        }
    }
    ...
    for &(dx, dy) in &[(0.0f32,-1.0),(0.0,1.0),(-1.0,0.0),(1.0,0.0),(-0.707,-0.707),(0.707,-0.707),(-0.707,0.707),(0.707,0.707)] {
        let gap = sr + 3.0;
        draw_line(sx + dx*gap, sy + dy*gap, sx + dx*(gap+6.0), sy + dy*(gap+6.0), 2.0, ray_color);
    }
    draw_rectangle(sx - sr, sy - sr, sr * 2.0, sr * 2.0, Color::from_rgba(255, 230, 80, 255));
    for &(cx, cy) in &[(sx-sr,sy-sr),(sx+sr-2.0,sy-sr),(sx-sr,sy+sr-2.0),(sx+sr-2.0,sy+sr-2.0)] {
        draw_rectangle(cx, cy, 2.0, 2.0, Color::from_rgba(220, 170, 80, 255));
    }

    let ground_y = GAME_H * 0.5;
    draw_rectangle(60.0,  ground_y + 2.0, 10.0, 5.0, Color::from_rgba(90, 60, 20, 255));
    draw_rectangle(150.0, ground_y + 2.0, 14.0, 4.0, Color::from_rgba(85, 55, 18, 255));
    draw_rectangle(200.0, ground_y + 3.0,  8.0, 3.0, Color::from_rgba(95, 62, 22, 255));
}
