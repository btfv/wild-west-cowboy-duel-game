...
impl WaitingScreen {
    pub fn new(url: &str) -> Self {
        ...
        if let Some(ref t) = qr_tex { t.set_filter(FilterMode::Nearest); }
        Self { qr_tex }
    }
}
...
impl Screen for WaitingScreen {
    fn draw(&mut self, ctx: &ScreenContext) -> Vec<GameEvent> {
        ...
        if let Some(ref tex) = self.qr_tex {
            ...
            draw_texture_ex(tex, GAME_W / 2.0 - qr_size / 2.0, 50.0, WHITE, DrawTextureParams {
                dest_size: Some(vec2(qr_size, qr_size)), ..Default::default()
            });
        }
